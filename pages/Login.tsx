  ...
  const loginWithGoogle = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/#/new-sale`,
        },
      });
          ...
          <div className="w-full max-w-[420px] mx-auto">
            ...
            <div className="bg-[#0f0f0f] border border-white/5 p-8 rounded-[32px] shadow-2xl">
              <button
                ...
                type="button"
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 bg-transparent border border-white/10 py-3.5 rounded-2xl hover:bg-white/5 transition-all duration-200 font-medium text-sm text-white disabled:opacity-50"
              >
