            ...
            <div className="space-y-3">
                {Object.entries(byMethod).map(([method, value]) => (
                    <div key={method} className="flex justify-between items-center text-sm">
                         ...
                         <span className="font-bold dark:text-white">R$ {(value as number).toFixed(2)}</span>
                    </div>
                ))}
            </div>
        </div>
               ...
               <div key={sale.id} className="bg-white dark:bg-gray-900 p-4 rounded-xl shadow-sm border-l-4 border-purple-500 flex justify-between items-center">
                  <div>
                     ...
                     <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                       <Clock className="w-3 h-3" /> {new Date(sale.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                     </p>
                  </div>
